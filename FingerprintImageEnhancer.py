...
class FingerprintImageEnhancer(object):
    def __init__(self):
        self.ridge_segment_blksze = 38
        self.ridge_segment_thresh = 0.2
        self.gradient_sigma = 1
        self.block_sigma = 7
        self.orient_smooth_sigma = 3
        self.ridge_freq_blksze = 22
        self.ridge_freq_windsze = 3
    ...
    def __ridge_segment(self, img):
        ...
        ''''''
        new_rows = np.int_(self.ridge_segment_blksze * np.ceil((np.float_(rows)) / (np.float_(self.ridge_segment_blksze))))
        new_cols = np.int_(self.ridge_segment_blksze * np.ceil((np.float_(cols)) / (np.float_(self.ridge_segment_blksze))))
